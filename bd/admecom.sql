...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 19-06-2026 a las 02:15:24
...
CREATE TABLE `gasto` (
  ...
  `FECHA` date NOT NULL,
  `RELA_CATEGORIA_GASTO` int(2) NOT NULL,
  `RELA_INGRESO` int(11) NOT NULL,
...
CREATE TABLE `ingreso` (
  ...
  `RELA_ORIGEN_INGRESO` int(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish2_ci;
...
CREATE TABLE `origen_ingreso` (
  `ID_ORIGEN_INGRESO` int(11) NOT NULL,
...
INSERT INTO `origen_ingreso` (`ID_ORIGEN_INGRESO`, `ORIGEN`) VALUES
...
ALTER TABLE `gasto`
  ...
  ADD KEY `ID_CATEGORIA_GASTO` (`RELA_CATEGORIA_GASTO`),
  ADD KEY `ID_INGRESO` (`RELA_INGRESO`);
...
ALTER TABLE `ingreso`
  ...
  ADD KEY `RELA_ORIGEN_INGRESO` (`RELA_ORIGEN_INGRESO`);

--
-- Indices de la tabla `origen_ingreso`
--
ALTER TABLE `origen_ingreso`
  ADD PRIMARY KEY (`ID_ORIGEN_INGRESO`);
...
--
ALTER TABLE `origen_ingreso`
  MODIFY `ID_ORIGEN_INGRESO` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `gasto`
  ADD CONSTRAINT `FK_GASTO_CATEGORIA_GASTO` FOREIGN KEY (`RELA_CATEGORIA_GASTO`) REFERENCES `categoria_gasto` (`ID_CATEGORIA_GASTO`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_GASTO_INGRESO` FOREIGN KEY (`RELA_INGRESO`) REFERENCES `ingreso` (`ID_INGRESO`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Filtros para la tabla `ingreso`
--
ALTER TABLE `ingreso`
  ADD CONSTRAINT `FK_INGRESO_ORIGEN_INGRESO` FOREIGN KEY (`RELA_ORIGEN_INGRESO`) REFERENCES `origen_ingreso` (`ID_ORIGEN_INGRESO`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
